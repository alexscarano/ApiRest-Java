...
public class ApiServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
       ...
       response.setContentType("application/json;charset=UTF-8");
        try {
            ...
            response.sendError(500, "Internal server error: " + e.getLocalizedMessage());
        }
    ...
    private void processSession(JSONObject file, HttpServletRequest request, HttpServletResponse response) throws Exception {
        if (request.getMethod().toLowerCase().equals("put")){
            ...
            User userObj = User.getUser(login, password);
           
            if (userObj != null){
                request.getSession().setAttribute("user", userObj);
                file.put("id", userObj.getRowId());
                file.put("login", userObj.getLogin());
                file.put("name", userObj.getName());
                file.put("role", userObj.getRole());
                file.put("passwordHash", userObj.getPasswordHash());
    ...
    private void processUsers(JSONObject file, HttpServletRequest request, HttpServletResponse response) throws Exception {        
        if (request.getSession().getAttribute("user") == null ){
            ...
            // Deleta usuário com base no parametro, pois o método delete não suporta body
    ...
    private void processParking(JSONObject file, HttpServletRequest request, HttpServletResponse response) throws Exception {
        if (request.getSession().getAttribute("user") == null ){
            ...
            VehicleStay.insertVehicleStay(model, color, plate);
        }
        else if (request.getMethod().toLowerCase().equals("put")){
            JSONObject body = getJsonBody(request.getReader());
